...
export function ShareTripProfessional({ 
  ...
  const shareText = `Check out my AI-planned trip to ${destination}! ${days} days, ${travelers} travelers, ₹${budget.toLocaleString('en-IN')} budget. Planned with TRAVELOPEDIA 🌍✈️`;
    ...
    <Card className="border border-gray-200/60 dark:border-gray-800 bg-gradient-to-br from-white to-gray-50/50 dark:from-gray-900/60 dark:to-gray-900/10 shadow-md overflow-hidden relative">
      ...
      <CardContent className="p-8 space-y-7">
        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[#0055CC]/15 dark:bg-[#0055CC]/10 flex items-center justify-center">
            <Share2 className="w-5 h-5 text-[#0055CC] dark:text-blue-400" />
          </div>
          <div>
        ...
        <div className="p-5 bg-gradient-to-r from-blue-50/50 to-[#FF6B35]/5 dark:from-blue-950/20 dark:to-orange-950/10 rounded-2xl border border-blue-100/50 dark:border-gray-800/80 shadow-inner">
          ...
          <div className="flex gap-2">
            <Input
              ...
              value={tripUrl}
              readOnly
              className="flex-1 px-4 h-12 bg-white dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-xl text-sm font-mono text-gray-600 dark:text-gray-300 focus:outline-none focus:border-blue-500"
            />
            <Button
              onClick={handleCopyLink}
              className="h-12 bg-[#0055CC] hover:bg-blue-700 text-white font-bold px-6 rounded-xl shadow-md transition flex items-center gap-2 whitespace-nowrap"
            >
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {platforms.map((platform) => (
              <a
                ...
                className={`flex flex-col items-center gap-3 p-4 bg-white dark:bg-gray-950/45 border border-gray-200/60 dark:border-gray-800/80 rounded-xl transition-all hover:-translate-y-0.5 hover:shadow-md ${platform.color} group`}
              >
                <div className={`w-11 h-11 rounded-full ${platform.bgGlow}/10 dark:${platform.bgGlow}/5 ${platform.textColor} flex items-center justify-center group-hover:scale-110 transition-transform`}>
                  {platform.svg}
                </div>
                <span className={`text-xs font-bold text-gray-700 dark:text-gray-400 group-hover:${platform.textColor} transition-colors`}>
                  {platform.name}
                </span>
              </a>
            ))}
        ...
        {/* Social Post Preview */}
        <div className="p-4 bg-gray-50/50 dark:bg-gray-950 border border-gray-200/60 dark:border-gray-800 rounded-xl space-y-1 text-sm shadow-inner">
        ...
        {/* Footer Info */}
        <div className="pt-5 border-t border-gray-200/60 dark:border-gray-800/80 text-center">
